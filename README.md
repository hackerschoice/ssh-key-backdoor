...
This program generates a backdoor that hides inside an OpenSSH _public_ key (e.g. `id_rsa.pub` or `authorized_keys`). The backdoor will execute when the user logs in next.
...
Prepend the output tp the *public* `id_rsa.pub` file like so:
```shell
